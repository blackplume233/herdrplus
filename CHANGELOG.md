...
- **右键菜单会自己收了**：以前只有「侧栏里的点击」能让菜单消失，点到编辑器/终端/标题栏（都在 webview 之外）侧栏收不到点击，菜单就挂在那儿。现在**失焦、滚轮滚动、窗口尺寸变化**都收起（菜单项选中后收起不变）。
- **「在当前 workspace 新开一个终端」开到对的 workspace 上**：它原来读本地快照的 `focused_workspace_id`，刚点过侧栏行 / 刚用 CLI 切过焦点时快照还是旧的，终端就开到**上一个** workspace 上去了（名字能看出来）。现在先刷新再读，并回退到最近点选的那一行。
- QA：修掉一条**从来没跑起来的断言**——它在 `ops` 初始化之前引用 `ops`（TDZ 直接抛 ReferenceError），「手上没有内嵌终端时点 workspace 行 → 自动开一个并钉住」这条契约因此从未被验证，也就漏了上面第一个 bug；新增「菜单在点到 webview 之外后收起」「没有 herdr 终端时点行会自动开一个」两条断言。
