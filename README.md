...
| **点 workspace 行主体** | 切到该 workspace | **乐观**：当前标识立刻移动（不等服务端往返），快照回来自然纠正；内嵌终端随后跟随 |
| **点 pane 行主体**（Agents view 任意行、workspace 展开后的子行） | 跳到那个 pane | 同上；`focusPane` → 服务端 `focused_pane_id` |
| **行首 `▸/▾`**（workspace 行） | 展开/折叠该 workspace 的**子 panel**（它的 pane，一行一个） | 展开集合按 webview 记忆；默认全折叠 |
...
| workspace 行 hover：`◫` | 预览该行当前 pane 的输出 | 开在当前栏；不可见时暂停刷新 |
| workspace 行 hover：`✕` | 关闭这个 workspace（**只关容器，不删目录**） | 里面有没结束的 agent 时**先出内联确认条** |
| pane 行 hover：`▣` / `◫` | 跳到这个 pane / 预览这个 pane 输出（**开在当前编辑器栏**，不新开栏） | 与点行主体不同：`▣` 明确表示「跳过去」 |
| 行右键（workspace） | `打开 / 聚焦 herdr 终端`（全局）、`重命名…`、`关闭 workspace`（红色） | Esc / 点别处关闭；越界自动夹回视口 |
| 行右键（pane） | `跳到这个 pane`、`预览输出（当前栏）`、`在新栏打开预览`、`重命名 pane…`、`关闭 pane`（红色，会结束进程） | 「在新栏打开预览」是并排看多个 pane 的方式；关 pane **先出确认条** |
| 确认条 | 破坏性操作的二次确认（关 workspace / 关 pane / 归档关闭） | 按钮文案 `仍然关闭` / `关闭 pane` / `关闭这些` + `取消`；Esc = 取消，Enter = 确认 |

**设计原则**：行内只留「指向该行」的高频动作（切过去、起 Agent、预览、关掉）；全局动作在 view 标题上（VSCode 原生的 title action，列表折叠时也够得着），低频动作（重命名）在右键菜单。
...
| 中间区 | `herdr` 终端（编辑器区或面板，可配）；**侧栏点击 → 内嵌终端跟着切到那个 workspace**；要并列看两个就 `Herdr: 新开 herdr 终端视图` 或 `Ctrl+Alt+P` 预览输出 |
| 终端 tab 右键 | 启动 Agent（新终端）/ 预览 Pane 输出 |
...
| `Ctrl+Alt+P` | 预览 Pane 输出（当前栏；一个 pane 一个看板，新栏用右键菜单） |
...
| 想要的效果 | 用哪个 | 同时性 | 可交互 |
| --- | --- | --- | --- |
| 几个标签页各自记住一个位置，切标签即切过去 | `Herdr: 新开 herdr 终端视图` → 选一个 workspace（或用 workspace 行右键 → `新开终端并钉在这个 workspace`） | 否（服务端焦点切过去） | ✅ 真 TUI |
| 同时盯多个 agent / pane | 行内 `◫` 开在当前栏；pane 行右键 → `在新栏打开预览` 开在旁边（**一个 pane 一个看板**） | ✅ | ❌ 只读 |
